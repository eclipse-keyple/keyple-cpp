...
const std::shared_ptr<Logger> logger = LoggerFactory::getLogger(typeid(UseCase_Calypso3_Rev1Selection_Pcsc));
    ...
    if (poReader->isSePresent()) {
        ...
        std::shared_ptr<PoSelectionRequest> poSelectionRequest =
            std::make_shared<PoSelectionRequest>(
                std::make_shared<PoSelector>(SeCommonProtocols::PROTOCOL_ISO14443_4,
                                             std::make_shared<PoSelector::PoAtrFilter>(poAtrRegex), nullptr,
                                             StringHelper::formatSimple("ATR: ", poAtrRegex)), ChannelState::KEEP_OPEN);

        /*
         * Prepare the selection of the DF RT.
         */
        std::vector<char> dfrt = ByteArrayUtil::fromHex(poDfRtPath);
        int selectFileParserIndex = poSelectionRequest->prepareSelectFileCmd(dfrt,
        ...
        if (selectionResult->hasActiveSelection()) {
            ...
            std::shared_ptr<SelectFileRespPars> selectFileRespPars =
                std::dynamic_pointer_cast<SelectFileRespPars>(matchingSelection->getResponseParser(selectFileParserIndex));
