...
: public std::enable_shared_from_this<UseCase_Calypso2_DefaultSelectionNotification_Pcsc>, public ObservableReader::ReaderObserver {
private:
    /**
     *
     */
    const std::shared_ptr<Logger> logger = LoggerFactory::getLogger(typeid(UseCase_Calypso2_DefaultSelectionNotification_Pcsc));
...
public:
    ...
    UseCase_Calypso2_DefaultSelectionNotification_Pcsc()
    {
         ...
         */
        readEnvironmentParserIndex = poSelectionRequest->prepareReadRecordsCmd(CalypsoClassicInfo::SFI_EnvironmentAndHolder,
        ...
        std::shared_ptr<UseCase_Calypso2_DefaultSelectionNotification_Pcsc> shared_this(this);
    ...
    void update(std::shared_ptr<ReaderEvent> event)
    {
        ...
        if (type == ReaderEvent::EventType::SE_MATCHED) {
            std::shared_ptr<MatchingSelection> matchingSelection
                = seSelection->processDefaultSelection(event->getDefaultSelectionsResponse())->getActiveSelection();
