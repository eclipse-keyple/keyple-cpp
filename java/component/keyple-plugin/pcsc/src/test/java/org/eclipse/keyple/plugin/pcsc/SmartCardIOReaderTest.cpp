...
#include "../../../../../../../main/java/org/eclipse/keyple/plugin/pcsc/PcscReader.h"
...
//ORIGINAL LINE: @Before public void setUp() throws CardException, IllegalArgumentException, KeypleBaseException
                    void SmartCardIOReaderTest::setUp() throw(CardException, std::invalid_argument, KeypleBaseException) {
                        when(terminal->connect(any(std::string::typeid))).thenReturn(card);
                        ...
                        res = std::make_shared<ResponseAPDU>(responseApduByte);

                        readerName = "reader";
                        reader = std::make_shared<PcscReader>("pcscPlugin", terminal);
                        reader->setParameter(PcscReader::SETTING_KEY_LOGGING, "true");
                    }

//JAVA TO C++ CONVERTER TODO TASK: Most Java annotations will not have direct C++ equivalents:
//ORIGINAL LINE: @Test public void testSmartCardIOReader()
                    void SmartCardIOReaderTest::testSmartCardIOReader() {
