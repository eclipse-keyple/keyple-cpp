    ...
    std::shared_ptr<SeResponse> seResponse =
        std::make_shared<SeResponse>(
            ...
            std::make_shared<SelectionStatus>(
                nullptr, std::make_shared<ApduResponse>(resp, nullptr), true),
            responses);

    CloseSessionRespPars apduResponseParser(seResponse->getApduResponses()[0],
                                            nullptr);
...
TEST(CloseSessionRespParsTest, TestToPOHalfSessionSignature)
{
    ...
    const std::vector<uint8_t> apduResponseCaseTwo = {
        0xA8, 0x31, 0xC3, 0x3E, 0xA7, 0x21, 0xC2, 0x2E, 0x90, 0x00};
    const std::vector<uint8_t> apduResponseCaseThree = {
        0xA8, 0x31, 0xC3, 0x90, 0x00};
    ...
    /* Case Length = 4 */
    CloseSessionRespPars pars1(
        std::make_shared<ApduResponse>(apduResponse, nullptr), nullptr);
    ASSERT_EQ(pars1.getSignatureLo(), sessionSignature);

    /* Case Length = 8 */
    CloseSessionRespPars pars2(
        std::make_shared<ApduResponse>(apduResponseCaseTwo, nullptr),nullptr);
    ASSERT_EQ(pars2.getSignatureLo(), sessionSignatureCaseTwo);

    /* Case Other */
    EXPECT_THROW(
        std::make_shared<CloseSessionRespPars>(
            std::make_shared<ApduResponse>(apduResponseCaseThree, nullptr),
            nullptr),
...
TEST(CloseSessionRespParsTest, existingTestConverted)
{
    ...
    CloseSessionRespPars parser(
        std::make_shared<ApduResponse>(resp, nullptr), nullptr);
...
TEST(CloseSessionRespParsTest, abortingASession)
{
    ...
    CloseSessionRespPars parser(std::make_shared<ApduResponse>(resp, nullptr),
                                nullptr);
}
...
TEST(CloseSessionRespParsTest, lc4WithPostponedData)
{
    const std::vector<uint8_t> resp =
        ByteArrayUtil::fromHex("04 345678 FEDCBA98 9000");
    CloseSessionRespPars parser(std::make_shared<ApduResponse>(resp, nullptr),
                                nullptr);

    ASSERT_EQ(ByteArrayUtil::toHex(parser.getSignatureLo()), "FEDCBA98");
    ASSERT_EQ(ByteArrayUtil::toHex(parser.getPostponedData()), "04345678");
}
