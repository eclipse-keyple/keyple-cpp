...
ReadRecordsRespPars::ReadRecordsRespPars(std::shared_ptr<ApduResponse> apduResponse,
                                         ReadDataStructure readDataStructure, char recordNumber)
: AbstractPoResponseParser(apduResponse)
{
        ...
        switch (readDataStructure) {
            ...
            case ReadDataStructure::MULTIPLE_RECORD_DATA: {
                ...
                for (std::set<int>::const_iterator it = records.begin(); it != records.end(); ++it) {
                    int record = (int) *it;
                    sb->append(StringHelper::formatSimple("{RECORD = %d, DATA = %s}", record, ByteArrayUtil::toHex(recordMap->find(record)->second)));
                    if (it != records.end()) {
                        sb->append(", ");
                    }
                }
