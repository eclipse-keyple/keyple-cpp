...
const std::map<int, std::shared_ptr<StatusProperties>> SvUndebitRespPars::STATUS_TABLE = {
    {
        0x6400,
...
SvUndebitRespPars::SvUndebitRespPars(std::shared_ptr<ApduResponse> response,
      ...
      reinterpret_cast<AbstractPoCommandBuilder<AbstractPoResponseParser>*>(builder))
{
    ...
    if (response->getDataOut().size() != 0 &&
        ...
        throw IllegalStateException("Bad length in response to SV Undebit command.");
}

const std::map<int, std::shared_ptr<StatusProperties>>& SvUndebitRespPars::getStatusTable() const
{
    return STATUS_TABLE;
}

const std::vector<uint8_t> SvUndebitRespPars::getSignatureLo() const
{
    return getApduResponse()->getDataOut();
}

const std::string SvUndebitRespPars::toString() const
{
    return "SV Undebit";
}
