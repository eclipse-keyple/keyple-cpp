...
const std::map<int, std::shared_ptr<StatusProperties>> RehabilitateRespPars::STATUS_TABLE = {
    {
         0x6400,
...
RehabilitateRespPars::RehabilitateRespPars(std::shared_ptr<ApduResponse> response,
                                           RehabilitateCmdBuild* builder)
: AbstractPoResponseParser(
    response,
    reinterpret_cast<AbstractPoCommandBuilder<AbstractPoResponseParser>*>(builder)) {}

const std::map<int, std::shared_ptr<StatusProperties>>& RehabilitateRespPars::getStatusTable()
    const
{
    return STATUS_TABLE;
}
