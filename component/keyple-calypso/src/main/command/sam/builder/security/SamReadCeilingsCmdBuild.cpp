...
SamReadCeilingsCmdBuild::SamReadCeilingsCmdBuild(const SamRevision& revision,
    ...
    nullptr)
{
    ...
    if (operationType == CeilingsOperationType::CEILING_RECORD) {
        if (static_cast<int8_t>(index) < 0 || static_cast<int8_t>(index) > MAX_CEILING_REC_NUMB)
...
std::shared_ptr<SamReadCeilingsRespPars> SamReadCeilingsCmdBuild::createResponseParser(
