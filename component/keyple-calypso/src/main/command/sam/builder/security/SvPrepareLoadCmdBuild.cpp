...
SvPrepareLoadCmdBuild::SvPrepareLoadCmdBuild(const SamRevision& samRevision,
...
: AbstractSamCommandBuilder(
    std::make_shared<CalypsoSamCommand>(CalypsoSamCommand::SV_PREPARE_LOAD),
    nullptr)
{
    const uint8_t cla = samRevision.getClassByte();
    const uint8_t p1 = 0x01;
    const uint8_t p2 = 0xFF;
    std::vector<uint8_t> data(19 + svGetData.size()); /* header(4) + SvReload data (15) = 19 */

    System::arraycopy(svGetHeader, 0, data, 0, 4);
    System::arraycopy(svGetData, 0, data, 4, svGetData.size());
    System::arraycopy(svReloadCmdBuildData, 0, data, 4 + svGetData.size(),
                      svReloadCmdBuildData.size());

    mRequest = setApduRequest(cla, mCommand, p1, p2, data);
}

std::shared_ptr<SvPrepareOperationRespPars> SvPrepareLoadCmdBuild::createResponseParser(
