...
DigestUpdateMultipleCmdBuild::DigestUpdateMultipleCmdBuild(const SamRevision revision,
                                                           const bool encryptedSession,
                                                           const std::vector<uint8_t>& digestData)
: AbstractSamCommandBuilder(
    std::make_shared<CalypsoSamCommand>(CalypsoSamCommand::DIGEST_UPDATE_MULTIPLE),
    nullptr)
{
    ...
    if (static_cast<int>(digestData.size()) == 0 || static_cast<int>(digestData.size()) > 255)
            throw IllegalArgumentException("Digest data null or too long!");

    mRequest = setApduRequest(cla, mCommand, p1, p2, digestData);
}

std::shared_ptr<DigestUpdateMultipleRespPars> DigestUpdateMultipleCmdBuild::createResponseParser(
