...
: AbstractSamCommandBuilder(
    std::make_shared<CalypsoSamCommand>(CalypsoSamCommand::SV_PREPARE_UNDEBIT),
    nullptr)
{
    const uint8_t cla = samRevision.getClassByte();
    const uint8_t p1 = 0x01;
    const uint8_t p2 = 0xFF;
    std::vector<uint8_t> data(16 + svGetData.size()); /* header(4) + SvDebit data (12) = 16 */

    System::arraycopy(svGetHeader, 0, data, 0, 4);
    System::arraycopy(svGetData, 0, data, 4, svGetData.size());
    System::arraycopy(svUndebitCmdBuildDebitCmdBuildData, 0, data, 4 + svGetData.size(),
                      svUndebitCmdBuildDebitCmdBuildData.size());

    mRequest = setApduRequest(cla, mCommand, p1, p2, data);
}

std::shared_ptr<SvPrepareOperationRespPars> SvPrepareUndebitCmdBuild::createResponseParser(
