...
public:
    ...
    MOCK_METHOD(void, terminateSeCommunication, (), (override));
...
private:
    const std::string mName = "ALRS_AbstractLocalReaderMock";
};
...
private:
    const std::string mName = "ALRS_SmartSelectionReaderMock";
};
...
TEST(AbstractLocalReaderSelectionTest, add_SeProtocolSetting)
{
    ...
    auto it = r.getProtocolsMap().find(SeCommonProtocols::PROTOCOL_ISO14443_4);
    ASSERT_EQ(it->second, protocolRule);
}
