...
public:
     ...
     * implements the {@link SmartRemovalReader} interface. The reader
     * implementation must manage the SE removal process itself. (for example by
     * using the analogous waitForCardAbsent method in the case of a plugin
     * based on smartcard.io [PC/SC]).
     * <p>
     * In the case where the reader plugin is not able to handle the SE removal
     * process itself (not implementing the {@link SmartRemovalReader}
     * interface, then it is managed by the isSePresentPing method defined in
     * this class.
     * <p>
