...
/**
 * Abstract definition of an threader local reader. Factorizes the observation
 * mechanism through the implementation of a monitoring thread.
 */
class EXPORT AbstractThreadedLocalReader
: public AbstractSelectionLocalReader, public Object {
protected:
    /**
    ...
    /**
     * Waits for a card. Returns true if a card is detected before the end of
     * the provided timeout.  Returns false if no card detected within the
     * delay.
     *
     * @param timeout the delay in millisecond we wait for a card insertion
     * @return presence status
     * @throws NoStackTraceThrowable a exception without stack trace in order to
     *         be catched and processed silently
     */
    virtual bool waitForCardPresent(long long timeout) = 0;

    /**
     * Wait until the card disappears. Returns true if a card has disappeared
     * before the end of the provided timeout. Returns false if the is still
     * present within the delay. Closes the physical channel when the card has
     * disappeared.
     *
     * @param timeout the delay in millisecond we wait for a card to be
     *        withdrawn
     * @return presence status
     * @throws NoStackTraceThrowable a exception without stack trace in order to
     *         be catched and processed silently
