...
const std::string IOSReaderImpl::PROTOCOL_T0   = "T=0";
const std::string IOSReaderImpl::PROTOCOL_T1   = "T=1";
const std::string IOSReaderImpl::PROTOCOL_T_CL = "T=CL";
const std::string IOSReaderImpl::PROTOCOL_ANY  = "T=0";

IOSReaderImpl::IOSReaderImpl(const std::string& pluginName, const IOSTerminal& terminal)
  ...
  mTransmissionMode(TransmissionMode::NONE)
{
    ...
    mLogger->debug("[IOSReaderImpl] constructor => using terminal %\n", terminal.getName());

    /*
     * Using null values to use the standard method for defining default values
     *
     * /!\ Must use hardcoded strings here as
     *     IOSReader::SETTING_KEY_TRANSMISSION_MODE and the three other strings
...
std::shared_ptr<ObservableReaderStateService> IOSReaderImpl::initStateService()
{
...
void IOSReaderImpl::closePhysicalChannel()
{
    try {
...
bool IOSReaderImpl::checkSePresence()
{
    try {
...
bool IOSReaderImpl::waitForCardPresent()
{
    if (mShuttingDown)
        return false;
    ...
    try {
        ...
        while (mLoopWaitSe) {
            ...
            if (mTerminal.didUserCancelOrTimeout()) {
                mLogger->trace("[%] user cancelled or timeout\n", getName());
                switchState(MonitoringState::WAIT_FOR_START_DETECTION);
                dynamic_cast<ObservableReader*>(this)->clearObservers();
                return false;
            }
...
void IOSReaderImpl::stopWaitForCard()
{
    ...
    mTerminal.stopWaitForCard();
}

bool IOSReaderImpl::waitForCardAbsentNative()
{
    if (mShuttingDown)
        return false;
...
void IOSReaderImpl::stopWaitForCardRemoval()
{
    mLoopWaitSeRemoval = false;
}

std::vector<uint8_t> IOSReaderImpl::transmitApdu(const std::vector<uint8_t>& apduIn)
{
    ...
    try {
        ...
        mLogger->error("transmitApdu - IOSTerminalException %\n", e);
...
bool IOSReaderImpl::protocolFlagMatches(const std::shared_ptr<SeProtocol> protocolFlag)
{
    bool result;
...
void IOSReaderImpl::setParameter(const std::string& name,
                                  const std::string& value)
{    mLogger->debug("[%] setParameter => IOS: Set a parameter. NAME = %,"
                  " VALUE = %\n", getName(), name, value);
     ...
     *     are not yet instantiated.
     */
    if (name == IOSReader::SETTING_KEY_TRANSMISSION_MODE) {
        if (value == "")
            mTransmissionMode = TransmissionMode::NONE;
    ...
    } else if (name == IOSReader::SETTING_KEY_PROTOCOL) {
        ...
        else if (value == IOSReader::SETTING_PROTOCOL_T0)
            mParameterCardProtocol = "T=0";
        else if (value == IOSReader::SETTING_PROTOCOL_T1)
            mParameterCardProtocol = "T=1";
        else if (value == IOSReader::SETTING_PROTOCOL_T_CL)
    ...
    } else if (name == IOSReader::SETTING_KEY_MODE) {
        if (value == "" || value == IOSReader::SETTING_MODE_SHARED) {
        ...
        } else if (value == IOSReader::SETTING_MODE_EXCLUSIVE) {
    ...
    } else if (name == IOSReader::SETTING_KEY_DISCONNECT) {
        if (value == "" || value == IOSReader::SETTING_DISCONNECT_RESET) {
            mCardReset = true;
        } else if (value == IOSReader::SETTING_DISCONNECT_UNPOWER) {
            mCardReset = false;
        } else if (value == IOSReader::SETTING_DISCONNECT_EJECT ||
                   value == IOSReader::SETTING_DISCONNECT_LEAVE) {
            throw IllegalArgumentException(
...
const std::map<const std::string, const std::string>&
    IOSReaderImpl::getParameters() const
{
    /*
...
const std::vector<uint8_t>& IOSReaderImpl::getATR()
{
    return mTerminal.getATR();
}

bool IOSReaderImpl::isPhysicalChannelOpen()
{
    return mChannelOpen;
}

void IOSReaderImpl::openPhysicalChannel()
{
    /*
...
const TransmissionMode& IOSReaderImpl::getTransmissionMode() const
{
    /*
     * /!\ Java diff
     *
