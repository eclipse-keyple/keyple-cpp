...
//    if (this->m_card == 0)
//    {
//        if (SCardConnect(this->m_context, this->m_name, this->m_share_mode, this->m_protocol, &this->m_card,
...
//            if (SCardGetAttrib(this->m_card, SCARD_ATTR_ATR_STRING, (uint8_t *)&atr, &cbyte) ==
//                SCARD_S_SUCCESS)
//            {
...
//    if (this->m_card != 0 && this->m_channel_openned == false)
//    {
//        ///
...
//        if (response.size() == 2 && apduRequest.isCase4() && response[response.size() - 2] == 0x90 &&
...
//        this->m_settings.insert(
//            std::pair<std::string, std::string>(PARAM_KEY_SHARE_MODE, PARAM_VALUE_SHARE_SHARED));
//        break;
//    case SCARD_SHARE_EXCLUSIVE:
//        DBG_INFO_MSG("Key = " << PARAM_KEY_SHARE_MODE << ", Value = " << PARAM_VALUE_SHARE_EXCLUSIVE);
//        this->m_settings.insert(
//            std::pair<std::string, std::string>(PARAM_KEY_SHARE_MODE, PARAM_VALUE_SHARE_EXCLUSIVE));
//        break;
//    case SCARD_SHARE_DIRECT:
//        DBG_INFO_MSG("Key = " << PARAM_KEY_SHARE_MODE << ", Value = " << PARAM_VALUE_SHARE_DIRECT);
//        this->m_settings.insert(
//            std::pair<std::string, std::string>(PARAM_KEY_SHARE_MODE, PARAM_VALUE_SHARE_DIRECT));
//        break;
//    }
//
//    // PROTOCOL
...
//        this->m_settings.insert(
//            std::pair<std::string, std::string>(PARAM_KEY_SHARE_MODE, PARAM_VALUE_PROTOCOL_T0));
//        break;
//    case SCARD_PROTOCOL_T1:
//        DBG_INFO_MSG("Key = " << PARAM_KEY_PROTOCOL << ", Value = " << PARAM_VALUE_PROTOCOL_T1);
//        this->m_settings.insert(
//            std::pair<std::string, std::string>(PARAM_KEY_SHARE_MODE, PARAM_VALUE_PROTOCOL_T1));
//        break;
//    case SCARD_PROTOCOL_T0 | SCARD_PROTOCOL_T1:
//        DBG_INFO_MSG("Key = " << PARAM_KEY_PROTOCOL << ", Value = " << PARAM_VALUE_PROTOCOL_T0T1);
//        this->m_settings.insert(
//            std::pair<std::string, std::string>(PARAM_KEY_SHARE_MODE, PARAM_VALUE_PROTOCOL_T0T1));
//        break;
//    }
//
//    // DISPOSITION
...
//        this->m_settings.insert(
//            std::pair<std::string, std::string>(PARAM_KEY_DISPOSITION, PARAM_VALUE_DISPOSITION_LEAVE));
//        break;
//    case SCARD_RESET_CARD:
//        DBG_INFO_MSG("Key = " << PARAM_KEY_DISPOSITION << ", Value = " << PARAM_VALUE_DISPOSITION_RESET);
//        this->m_settings.insert(
//            std::pair<std::string, std::string>(PARAM_KEY_DISPOSITION, PARAM_VALUE_DISPOSITION_RESET));
//        break;
//    case SCARD_UNPOWER_CARD:
//        DBG_INFO_MSG("Key = " << PARAM_KEY_DISPOSITION << ", Value = " << PARAM_VALUE_DISPOSITION_UNPOWER);
//        this->m_settings.insert(
//            std::pair<std::string, std::string>(PARAM_KEY_DISPOSITION, PARAM_VALUE_DISPOSITION_UNPOWER));
//        break;
//    case SCARD_EJECT_CARD:
//        DBG_INFO_MSG("Key = " << PARAM_KEY_DISPOSITION << ", Value = " << PARAM_VALUE_DISPOSITION_EJECT);
//        this->m_settings.insert(
//            std::pair<std::string, std::string>(PARAM_KEY_DISPOSITION, PARAM_VALUE_DISPOSITION_EJECT));
//        break;
//    }
//
...
//    DBG_TRACE_CALL();
//
//    for (std::list<ReaderObserver *>::iterator iterator_observer = m_observers_list.begin(),
//                                               end               = m_observers_list.end();
//         iterator_observer != end; ++iterator_observer)
//    {
...
//                    if (this->m_card == 0)
//                    {
//                        keyple::containers::ReaderEvent event(this,
//                                                              keyple::containers::ReaderEvent::SE_INSERTED);
//                        this->notifyObservers(event);
//                        DBG_INFO_MSG("Reader " << this->m_name << ": card is present. dwEventState = "
//                                               << std::hex << rgscState.dwEventState);
//                    }
//                }
//                else
//                {
//                    if (this->m_card != 0)
//                    {
//                        keyple::containers::ReaderEvent event(dynamic_cast<ObservableReader *>(this),
//                                                              keyple::containers::ReaderEvent::SE_REMOVAL);
//                        this->notifyObservers(event);
//                        DBG_INFO_MSG("Reader " << this->m_name << ": card is not present. dwEventState = "
//                                               << std::hex << rgscState.dwEventState);
//                    }
//                    this->m_card = 0;
//                }
...
//
//    lReturn = SCardTransmit(this->m_card, &scioReq, reinterpret_cast<uint8_t *>(apdu.data()), apdu.size(),
//                            NULL, responseBuffer, (unsigned long *)&responseApduLength);
//
//    if (lReturn == SCARD_S_SUCCESS)
//    {
//
//        if (responseApduLength == 2 &&
//            (responseBuffer[0] == 0x61 || responseBuffer[0] == 0x6C || responseBuffer[0] == 0x9F))
//        {
//            DBG_DUMP_HEX_VECTOR("RESPONSE: ",
//                                std::vector<uint8_t>(responseBuffer, responseBuffer + responseApduLength));
...
//            lReturn = SCardTransmit(this->m_card, &scioReq, getResponse, 5, NULL, responseBuffer,
...
//
//    selectApplicationAPDU.insert(std::end(selectApplicationAPDU), std::begin(aid), std::end(aid));
//    std::vector<uint8_t> response = scardTransmit(selectApplicationAPDU);
//    if (response.size() == 2 && response[0] == 0x90 && response[1] == 0x00)
//    {
//        response = getResponseCase4();
//    }
//    return response;
