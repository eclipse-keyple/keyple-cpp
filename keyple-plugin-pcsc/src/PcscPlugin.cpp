...
std::list<ProxyReader *> PcscPlugin::getReaders()
{
    ...
    lReturn = SCardListReaders(this->m_context, NULL, (LPTSTR)&pmszReaders, &cch);
    switch (lReturn)
    {
    ...
    case SCARD_S_SUCCESS:
        ...
        while ('\0' != *pReader)
        {
            ...
            m_readers_list.push_back(
                static_cast<ProxyReader *>(static_cast<ConfigurableReader *>(proxyReader)));

            /// Advance to the next value.
            pReader = pReader + strlen(pReader) + 1;
        }
        /// Free the memory.
        lReturn2 = SCardFreeMemory(m_context, pmszReaders);
